...
- 상태: 진행 중 (In Progress) — API·화면 완료, 런타임 실측 미완
- 날짜: 2026-08-29
...
- **화면이 실제로 그려지는 모습.** `admin.html`에 붙였고 태그 균형·id 참조·JS 구문은
  정적으로 검증했지만, **브라우저에서 뜬 것을 보지 못했다.** 도커 데몬이 하루 8번 죽어
  앱을 못 띄웠다(힙 1g로 줄이고 Spring의 compose 통합을 꺼도 마찬가지였으므로
  JVM 자원 압박도 그 통합 탓도 아니다).
