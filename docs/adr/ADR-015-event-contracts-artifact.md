# ADR-015. 이벤트 계약을 독립 아티팩트(pay-contracts)로 분리한다

> **번호 변경**: 원래 ADR-008이었다. `main`이 모듈러 모놀리스로 되돌아가면서 이 번호를 다른 결정에 다시 썼다 — `main`의 ADR-008은 「대사 불일치 수기 확정」다. 충돌을 피해 015로 옮겼다. 내용은 그대로다.
