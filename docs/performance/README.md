...
**① 완료된 이벤트가 지워지지 않는다.** Spring Modulith의 `completion-mode` 기본값은 `update`로,
완료 시 `completion_date`만 채우고 행은 영원히 남긴다. 발행량에 비례해 무한히 자란다.
...
- 완료분을 `event_publication_archive`로 옮긴다 → 핫 테이블에 크기 상한이 생긴다.
  `delete`가 아니라 `archive`를 고른 이유는 감사 흔적을 버리지 않기 위해서다.
