...
- 테스트: JUnit5 + Mockito. 기본 스위트 **568개** + 실 MySQL 통합 **13개**(`integrationTest`) + Toxiproxy 네트워크 카오스 **1개**(`chaosTest`) = 총 582개. 뒤의 둘은 컨테이너가 필요해 기본 스위트에서 제외하고 별도 태스크로 돌린다. Spring Modulith 경계 검증 포함. 락 전략 비교는 H2와 Testcontainers 실 MySQL에서 각각 재서 순위가 뒤집히는 것을 확인했다
...
- [docs/11 AI 운영 자동화 검토](docs/11-AI-운영자동화-검토.md): AI를 어디에 넣고 어디는 안 넣나 — 대사 원인 8개 중 6개는 산수, 데이터 도착 시점이 계층을 가른다, 해외·국내 사례 조사(Stripe·Adyen·PayPal·Nubank·Monzo·카카오뱅크·토스)로 판단 2건 정정
