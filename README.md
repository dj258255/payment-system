...
- 테스트: JUnit5 + Mockito, **532 tests** + Spring Modulith 경계 검증 + Toxiproxy 카오스(`chaosTest`). 락 전략 비교는 H2와 Testcontainers 실 MySQL에서 각각 재서 순위가 뒤집히는 것을 확인했다
...
- **정산**: 일 단위 배치 집계를 서비스 루프로 처리한다(대용량이면 Spring Batch로 확장 여지). 수수료율은
  ...
  skip**하며 법정공휴일은 미반영이다. 실서비스라면 공휴일 캘린더를 붙인다. 수수료를 원장 비용 계정으로
  분개하는 것은 후속 과제로 남겼다.
