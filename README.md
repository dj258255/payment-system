...
- 테스트: JUnit5 + Mockito. 기본 스위트 **614개** + 실 MySQL 통합 **13개**(`integrationTest`) + Toxiproxy 네트워크 카오스 **1개**(`chaosTest`) = 총 628개. 뒤의 둘은 컨테이너가 필요해 기본 스위트에서 제외하고 별도 태스크로 돌린다. Spring Modulith 경계 검증 포함. 락 전략 비교는 H2와 Testcontainers 실 MySQL에서 각각 재서 순위가 뒤집히는 것을 확인했다
...
실측 기록: [13 상담 초안 실측](docs/13-상담초안-실측.md) — 실데이터를 흘려 검증기 오반려 50%를 찾아 고친 과정
